...
exports.setUserRole = (0, https_1.onCall)(async (request) => {
    var _a, _b, _c, _d;
    firebase_functions_1.logger.info("🔧 setUserRole v5 - Versión Segura", { data: request.data });
    const callingUid = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid;
    const { userId, rol, nivelAcceso, puestoId } = request.data;
    // 1. Validar autenticación
    if (!callingUid) {
        throw new https_1.HttpsError("unauthenticated", "Usuario no autenticado para realizar esta acción.");
    }
    // 2. Validar datos de entrada
    if (!userId || !rol || !nivelAcceso) {
        firebase_functions_1.logger.error("❌ Datos de entrada inválidos para setUserRole", { data: request.data });
    ...
    try {
        ...
        if ((callerClaims === null || callerClaims === void 0 ? void 0 : callerClaims.rol) !== "Administrador") {
            firebase_functions_1.logger.warn("❌ Intento no autorizado para cambiar roles.", {
                callingUid,
                currentRole: (callerClaims === null || callerClaims === void 0 ? void 0 : callerClaims.rol) || "ninguno",
            ...
            throw new https_1.HttpsError("permission-denied", "No tiene permisos para ejecutar esta acción. Este intento ha sido registrado.");
        }
        firebase_functions_1.logger.info("✅ Autorizado por Custom Claims de Administrador", {
            adminId: callingUid,
            targetUserId: userId
        });
        ...
        if (callingUid === userId && rol !== "Administrador") {
            ...
            if (adminCountSnapshot.size <= 1) {
                throw new https_1.HttpsError("failed-precondition", "No puede quitarse el rol de Administrador porque es el último que queda.");
            }
        }
        ...
        if (puestoId) {
            ...
            if (puestoDoc.exists) {
                ...
                firebase_functions_1.logger.warn(`⚠️ Puesto ${puestoId} no encontrado.`);
            }
        }
        ...
        firebase_functions_1.logger.info("✅ Custom Claims asignados exitosamente.", { userId, claims: claimsToSet });
        ...
        await userDocRef.update({
            ...
            lastModifiedAt: firestore_2.Timestamp.now() // Rastrear cuándo se hizo
        });
        ...
        firebase_functions_1.logger.error("❌ Error fatal en setUserRole", { error, userId, callingUid });
 ...
 */
exports.initializeFirstAdmin = (0, https_1.onCall)(async (request) => {
    var _a, _b;
    firebase_functions_1.logger.info("🚀 initializeFirstAdmin - Intento de configuración inicial", { callingUid: (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid });
    const callingUid = (_b = request.auth) === null || _b === void 0 ? void 0 : _b.uid;
    ...
    try {
        ...
        if (!existingAdmins.empty) {
            firebase_functions_1.logger.warn("❌ Intento de inicializar admin cuando ya existe uno.", { attemptedBy: callingUid });
            await db.collection("security_events").add({
            ...
            throw new https_1.HttpsError("failed-precondition", "Ya existe al menos un administrador en el sistema.");
        }
        ...
        if (!expectedSecretKey || secretKey !== expectedSecretKey) {
            firebase_functions_1.logger.error("❌ Clave secreta para inicializar admin es inválida.", { callingUid });
            await db.collection("security_events").add({
                type: "invalid_secret_key_attempt",
                attemptedBy: callingUid,
                timestamp: new Date().toISOString()
            });
            throw new https_1.HttpsError("permission-denied", "Clave de inicialización inválida.");
        }
        // 4. Asignar rol de administrador
        const userDocRef = db.collection("users").doc(callingUid);
        const userDoc = await userDocRef.get();
        if (!userDoc.exists) {
            throw new https_1.HttpsError("not-found", "El perfil de usuario del solicitante no fue encontrado.");
        }
        const claimsVersion = Date.now();
        const adminClaims = {
            rol: "Administrador",
        ...
        await (0, auth_1.getAuth)().setCustomUserClaims(callingUid, adminClaims);
        await userDocRef.update({
            rol: "Administrador",
            nivelAcceso: "Confidencial",
            isFoundingAdmin: true,
            adminSince: firestore_2.Timestamp.now(),
            lastSyncAt: new Date().toISOString(),
            claimsVersion
        });
        firebase_functions_1.logger.info("✅ Primer administrador configurado exitosamente.", { adminId: callingUid });
        return {
            success: true,
        ...
        firebase_functions_1.logger.error("❌ Error en initializeFirstAdmin", { error, callingUid });
        if (error instanceof https_1.HttpsError) {
            throw error;
        }
        throw new https_1.HttpsError("internal", "Error crítico al configurar el primer administrador.");
    }
});
/**
...
exports.syncUserClaims = (0, https_1.onCall)(async (request) => {
    var _a;
    firebase_functions_1.logger.info("🔧 syncUserClaims - Sincronización iniciada por el usuario");
    const callingUid = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.uid;
    if (!callingUid) {
        throw new https_1.HttpsError("unauthenticated", "Usuario no autenticado");
    }
    try {
        ...
        const authUser = await (0, auth_1.getAuth)().getUser(callingUid);
        ...
        if (dbVersion !== tokenVersion || !tokenClaims.rol) {
            firebase_functions_1.logger.info("🔄 Desincronización detectada, corrigiendo...", { dbVersion, tokenVersion });
            ...
            firebase_functions_1.logger.info("✅ Claims sincronizados automáticamente para el usuario.", { uid: callingUid });
            return { success: true, wasSynced: true, message: "Tus permisos han sido sincronizados.", newClaimsVersion: claimsVersion };
        }
        firebase_functions_1.logger.info("✅ Claims ya están sincronizados.", { uid: callingUid });
        ...
        throw new https_1.HttpsError("internal", "Error al sincronizar tus permisos.");
    }
});
/**
...
exports.diagnoseClaimsHealth = (0, https_1.onCall)(async (request) => {
    ...
    if (((_b = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.token) === null || _b === void 0 ? void 0 : _b.rol) !== "Administrador") {
        throw new https_1.HttpsError("permission-denied", "Solo los administradores pueden ejecutar esta acción.");
    }
    try {
        const usersSnapshot = await db.collection("users").get();
        const problematicUsers = [];
        const promises = usersSnapshot.docs.map(userDoc => (0, auth_1.getAuth)().getUser(userDoc.id).then(authUser => {
            const userData = userDoc.data();
            const tokenClaims = authUser.customClaims || {};
            ...
            firebase_functions_1.logger.warn(`No se pudo verificar el usuario ${userDoc.id}:`, error.message);
        }));
        ...
        firebase_functions_1.logger.error("Error en diagnoseClaimsHealth:", error);
 ...
 */
exports.getSystemSecurityStatus = (0, https_1.onCall)(async (request) => {
    var _a, _b, _c;
    if (((_b = (_a = request.auth) === null || _a === void 0 ? void 0 : _a.token) === null || _b === void 0 ? void 0 : _b.rol) !== "Administrador") {
        throw new https_1.HttpsError("permission-denied", "Solo los administradores pueden ejecutar esta acción.");
    }
    try {
        ...
        return {
            ...
            firstAdminInitialized: ((_c = configDoc.data()) === null || _c === void 0 ? void 0 : _c.firstAdminInitialized) || false,
            recentSecurityEvents: recentEvents.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())))
        };
    }
    catch (error) {
        firebase_functions_1.logger.error("Error getting security status", { error });
        throw new https_1.HttpsError("internal", "Error al obtener estado de seguridad.");
    }
});
// =================================================================
...
exports.recalculateProcedure = (0, firestore_1.onDocumentWritten)("procedimientos/{docId}", async (event) => {
    ...
    if (!(after === null || after === void 0 ? void 0 : after.exists)) {
        firebase_functions_1.logger.info(`Procedimiento ${docId} eliminado, no se recalcula.`);
        return;
    }
    ...
    try {
        const activityDocs = await Promise.all(activityOrder.map((actId) => db.collection('actividades').doc(actId).get()));
        const activities = activityDocs.filter(doc => { var _a; return doc.exists && ((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.activa); }).map(doc => doc.data());
        let totalTiempo = 0;
        ...
        firebase_functions_1.logger.info(`✅ Procedimiento ${docId} recalculado: tiempo=${totalTiempo}, costo=${totalCosto} ${monedaFinal}`);
    }
    catch (error) {
...
exports.recalculateProcess = (0, firestore_1.onDocumentWritten)("procesos/{docId}", async (event) => {
    ...
    const after = (_a = event.data) === null || _a === void 0 ? void 0 : _a.after;
    if (!(after === null || after === void 0 ? void 0 : after.exists)) {
        firebase_functions_1.logger.info(`Proceso ${docId} eliminado, no se recalcula.`);
        return;
    }
    ...
    try {
        const procedureDocs = await Promise.all(procedimientoOrder.map((procId) => db.collection('procedimientos').doc(procId).get()));
        const procedures = procedureDocs.filter(doc => { var _a; return doc.exists && ((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.activo); }).map(doc => doc.data());
        let totalTiempo = 0;
        let totalCosto = 0;
        ...
        firebase_functions_1.logger.info(`✅ Proceso ${docId} recalculado: tiempo=${totalTiempo}, costo=${totalCosto} ${monedaFinal}`);
    }
    catch (error) {
...
exports.triggerRecalcFromActivity = (0, firestore_1.onDocumentWritten)("actividades/{docId}", async (event) => {
    ...
    const hasRelevantChanges = !((_c = event.data) === null || _c === void 0 ? void 0 : _c.before.exists) || !((_d = event.data) === null || _d === void 0 ? void 0 : _d.after.exists) || relevantFields.some(field => (beforeData === null || beforeData === void 0 ? void 0 : beforeData[field]) !== (afterData === null || afterData === void 0 ? void 0 : afterData[field]));
    ...
    for (const procedureId of affectedProcedureIds) {
        try {
            ...
            firebase_functions_1.logger.info(`✅ Disparado recálculo para procedimiento ${procedureId}`);
        }
        catch (error) {
            firebase_functions_1.logger.error(`❌ Error al disparar recálculo para proc ${procedureId}:`, error);
        }
    }
});
/**
 ...
 */
exports.triggerRecalcFromProcedure = (0, firestore_1.onDocumentWritten)("procedimientos/{docId}", async (event) => {
    var _a, _b, _c, _d;
    const docId = event.params.docId;
    const beforeData = (_a = event.data) === null || _a === void 0 ? void 0 : _a.before.data();
    const afterData = (_b = event.data) === null || _b === void 0 ? void 0 : _b.after.data();
    const financialFields = ['tiempoEstimado', 'costoEstimado', 'monedaCosto', 'activo'];
    const hasFinancialChanges = !((_c = event.data) === null || _c === void 0 ? void 0 : _c.before.exists) || !((_d = event.data) === null || _d === void 0 ? void 0 : _d.after.exists) || financialFields.some(field => (beforeData === null || beforeData === void 0 ? void 0 : beforeData[field]) !== (afterData === null || afterData === void 0 ? void 0 : afterData[field]));
    ...
    for (const processDoc of processesQuery.docs) {
        try {
            ...
            firebase_functions_1.logger.info(`✅ Disparado recálculo para proceso ${processDoc.id}`);
        }
        catch (error) {
            firebase_functions_1.logger.error(`❌ Error al disparar recálculo para proceso ${processDoc.id}:`, error);
        }
    }
});
